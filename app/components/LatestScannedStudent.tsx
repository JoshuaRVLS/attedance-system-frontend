    ...
    (async () => {
      if (lastStudent) {
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_URL}/api/v1/students/${lastStudent}`
        );
    ...
    <div className="relative font-montserrat pt-8 flex shadow-lg rounded-lg border border-secondary items-center flex-col min-h-[80dvh] min-w-96 max-w-[40rem]">
      {!studentData ? (
        ...
        <div className="flex w-full flex-col items-center">
          <Image
            alt="student_photo"
            className="object-fill size-[35rem] m-3 rounded-xl"
            src={`data:${studentData?.photo.mimetype};base64,${Buffer.from(
          ...
          <div className="flex flex-col items-center justify-between gap-16">
            ...
            <div>
              <span className="shadow-xl bg-primary p-2 rounded-full">
                Hadir pada{" "}
                {new Date(studentData.updatedAt).toLocaleString("id-ID", {
                  dateStyle: "full",
                  timeStyle: "long",
                })}
              </span>
